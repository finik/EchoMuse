...
async def trigger_voice_turn(
    ...
    Returns (should_continue, turn_ok, conversation_id):
      ...
      - turn_ok: True only if this turn produced a genuine spoken answer
        (trace.outcome == "ok" — STT got text, a pipeline ran, TTS played).
        False covers no_speech AND every failure outcome (stream_timeout,
        pipeline_refused, timeout, tts_error, no_tts) alike — deliberately:
        the controller uses this to decide whether to open a follow-up
        window, and a turn that merely picked up noise or errored out must
        not open one just because it wasn't a clean no-speech timeout (see
        PR #208 review — the earlier `no_speech_timeout_hit` check let
        exactly those failure outcomes through).
    ...
    All three are (False, False, conversation_id or "") if no active HA
    connection exists — treated the same as "no real answer" so the caller
    ...
    if server is None:
        ...
        return False, False, conversation_id or ""
